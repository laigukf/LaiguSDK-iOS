...
#pragma mark - 注意注意注意: 方法后面标记 style_deprecated ,代表该方法 推荐通过 LGChatViewStyle 调用

#define style_deprecated __attribute__((deprecated("修改 LGChatViewStyleType 中的对应属性代替")))

/**
 * @brief 客服聊天界面的配置类
 *
 * 开发者可以通过LGChatViewManager中提供的接口，来对客服聊天界面进行自定义配置；
 */
@interface LGChatViewManager : NSObject
...
/**
 * 设置收到消息的声音；
 * @param soundFileName 声音文件；如果要自定义声音，请将声音文件放在 LGChatViewAsset.bundle 中
 ...
 * @warning 如果开启同步，将会产生一定网络请求。所以建议顾客端只使用来鼓SDK的用户保持默认值
 ...
 *
 *  @warning 如果setLoginLGClientId接口，优先使用setLoginLGClientId来进行登录
