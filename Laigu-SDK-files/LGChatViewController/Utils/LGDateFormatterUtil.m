...
@implementation LGDateFormatterUtil
...
+ (LGDateFormatterUtil *)sharedFormatter
{
    ...
    dispatch_once(&onceToken, ^{
        _sharedFormatter = [[LGDateFormatterUtil alloc] init];
    });
...
{
    if ([LGDateFormatterUtil dateYearFromDate:date] == [LGDateFormatterUtil dateYearFromDate:[NSDate date]]) {
        NSInteger days = [LGDateFormatterUtil dateDayFromDate:[NSDate date]] - [LGDateFormatterUtil dateDayFromDate:date];
        if (days <= 1) {
            //昨天内
