...
+ (UIImage *)resizeImageToMaxScreenSize:(UIImage *)image {
    ...
    return [LGImageUtil resizeImage:image maxSize:screenSize];
}
...
+(UIImage *)blurryImage:(UIImage *)image withBlurLevel:(CGFloat)blur exclusionPath:(UIBezierPath *)exclusionPath
{
    ...
    if(pixelBuffer == NULL)
        NSLog(@"LaiGuSDK Error: No pixelbuffer");
    ...
    if (error) {
        NSLog(@"LaiGuSDK Error: error from convolution %ld", error);
    }
