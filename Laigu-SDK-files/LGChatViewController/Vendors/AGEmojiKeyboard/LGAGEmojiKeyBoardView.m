...
#import "LGAGEmojiKeyBoardView.h"
#import "LGAGEmojiPageView.h"
#import "UIImage+LGGenerate.h"
...
NSString *const LGRecentUsedEmojiCharactersKey = @"RecentUsedEmojiCharactersKey";


@interface LGAGEmojiKeyboardView () <UIScrollViewDelegate, LGAGEmojiPageViewDelegate>
...
@implementation LGAGEmojiKeyboardView

- (NSDictionary *)emojis {
  if (!_emojis) {
    NSBundle *selfBundle = [NSBundle bundleForClass:[self class]];
      NSString * fileRootPath = [[selfBundle bundlePath] stringByAppendingString:@"/LGChatViewAsset.bundle"];
      NSString * filePath = [fileRootPath stringByAppendingString:@"/LGEmojisList.plist"];
...
- (LGAGEmojiKeyboardViewCategoryImage)defaultSelectedCategory {
  ...
  return LGAGEmojiKeyboardViewCategoryImageFace;
}
...
- (NSArray *)imagesForSelectedSegments {
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (LGAGEmojiKeyboardViewCategoryImage i = LGAGEmojiKeyboardViewCategoryImageFace;
         i <= LGAGEmojiKeyboardViewCategoryImageCharacters;
         ++i) {
...
- (NSMutableArray *)recentEmojis {
  NSArray *emojis = [[NSUserDefaults standardUserDefaults] arrayForKey:LGRecentUsedEmojiCharactersKey];
...
- (void)setRecentEmojis:(NSMutableArray *)recentEmojis {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:recentEmojis forKey:LGRecentUsedEmojiCharactersKey];
}

- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<LGAGEmojiKeyboardViewDataSource>)dataSource {
...
- (UIImage *)segImageWithCategory:(LGAGEmojiKeyboardViewCategoryImage)category {
    NSString *imageName = @"";
    switch (category) {
//        case LGAGEmojiKeyboardViewCategoryImageRecent:
//            imageName = @"AGEmojiFlags";
//            break;
        case LGAGEmojiKeyboardViewCategoryImageFace:
            imageName = @"AGEmojiPeople";
            break;
        case LGAGEmojiKeyboardViewCategoryImageBell:
            imageName = @"AGEmojiObjects";
            break;
        case LGAGEmojiKeyboardViewCategoryImageFlower:
            imageName = @"AGEmojiAnimals";
            break;
        case LGAGEmojiKeyboardViewCategoryImageCar:
            imageName = @"AGEmojiPlaces";
            break;
        case LGAGEmojiKeyboardViewCategoryImageCharacters:
            imageName = @"AGEmojiSymbols";
            break;
    }
    UIImage *img = [[LGAssetUtil imageFromBundleWithName:imageName] addContentInsect:UIEdgeInsetsMake(7, 7, 7, 7)];
    img = [img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    return img;
}

- (UIImage *)segImageUnselectedWithCategory:(LGAGEmojiKeyboardViewCategoryImage)category {
    NSString *imageName = @"";
    switch (category) {
//        case LGAGEmojiKeyboardViewCategoryImageRecent:
//            imageName = @"AGEmojiFlags";
...
- (BOOL)requireToSetPageViewForIndex:(NSUInteger)index {
  ...
  for (LGAGEmojiPageView *page in self.pageViews) {
...
- (LGAGEmojiPageView *)synthesizeEmojiPageView {
  ...
  CGRect pageViewFrame = CGRectMake(0,
                                    ...
                                    CGRectGetHeight(self.emojiPagesScrollView.bounds));
  LGAGEmojiPageView *pageView = [[LGAGEmojiPageView alloc] initWithFrame: pageViewFrame
                                                backSpaceButtonImage:[LGAssetUtil imageFromBundleWithName:@"LGFileCancel"]
...
- (LGAGEmojiPageView *)usableEmojiPageView {
  LGAGEmojiPageView *pageView = nil;
  for (LGAGEmojiPageView *page in self.pageViews) {
...
- (void)setEmojiPageViewInScrollView:(UIScrollView *)scrollView atIndex:(NSUInteger)index {
  ...
  LGAGEmojiPageView *pageView = [self usableEmojiPageView];
...
- (void)emojiPageView:(LGAGEmojiPageView *)emojiPageView didUseEmoji:(NSString *)emoji {
  [self setInRecentsEmoji:emoji];
  [self.delegate emojiKeyBoardView:self didUseEmoji:emoji];
}

- (void)emojiPageViewDidPressBackSpace:(LGAGEmojiPageView *)emojiPageView {
