...
- (void)layoutViewWithFrame:(CGRect)frame{
    ...
    [emojiCollectionView registerClass:[LG_EmojiCell class] forCellWithReuseIdentifier:@"LG_EmojiCell"];
}
...
{
    LG_EmojiCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"LG_EmojiCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *sectionEmojiArr =  [emojiDic objectForKey:key];
    NSString * emojiStr = sectionEmojiArr[indexPath.row];
    if (self.inputViewDelegate && [self.inputViewDelegate respondsToSelector:@selector(LGInputViewObtainEmojiStr:)]) {
        [self.inputViewDelegate LGInputViewObtainEmojiStr:emojiStr];
    }
}
...
- (void)layoutBottomBt{
    ...
    [deleteBt setTitle:[LGBundleUtil localizedStringForKey:@"mq_delete"] forState:UIControlStateNormal];
    ...
    [returnBt setTitle:[LGBundleUtil localizedStringForKey:@"mq_send"] forState:UIControlStateNormal];
    ...
    for (int i = 0;i < emojiDic.allKeys.count;i ++) {
        ...
        UIImage *image = [LGAssetUtil imageFromBundleWithName:imageStr];
        UIButton *bt =  [LGUIMaker xlpInitWithFrame:CGRectMake(mmWidth + (aWidth * i), emojikeyboardHeight - bottomHeight,aWidth, bottomHeight) image:image backImage:nil corner:0 superView:self touchUpInside:nil];
        bt.tag = i + 10;
...
- (void)returnHandle:(UIButton *)bt{
    if (self.inputViewDelegate && [self.inputViewDelegate respondsToSelector:@selector(LGInputViewSendEmoji)]) {
        
        [self.inputViewDelegate LGInputViewSendEmoji];
    }
}
- (void)deleteHandle:(UIButton *)bt{
    if (self.inputViewDelegate && [self.inputViewDelegate respondsToSelector:@selector(LGInputViewDeleteEmoji)]) {
        
        [self.inputViewDelegate LGInputViewDeleteEmoji];
    }
}
@end
