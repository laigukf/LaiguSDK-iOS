#import "LAIGU_FBBitmapFontView.h"
#import "LAIGU_FBFontSymbol.h"
#import "LAIGU_FBBitmapFont.h"

#define UIColorFromRGB(rgbValue) [UIColor colorWithRed:((float)(((rgbValue) & 0xFF0000) >> 16))/255.0 green:((float)(((rgbValue) & 0xFF00) >> 8))/255.0 blue:((float)((rgbValue) & 0xFF))/255.0 alpha:1.0]

@implementation LAIGU_FBBitmapFontView
...
{
    _text = text;
    self.symbols = [LAIGU_FBFontSymbol symbolsForString:text];
...
- (NSInteger)numberOfHorizontalDot
{
    ...
    for (NSNumber *number in self.symbols) {
        ...
        totalDotsFromSymbols += [LAIGU_FBBitmapFont numberOfDotsWideForSymbol:symbol];
    }
...
//    CGFloat l = (self.edgeLength + self.margin) * (5 + self.numberOfPaddingDotsBetweenDigits);
    ...
    for (i = 0; i < [self.symbols count]; i++) {
        ...
        [LAIGU_FBBitmapFont drawSymbol:[[self.symbols objectAtIndex:i] intValue]
        ...
        CGFloat numberWide = [LAIGU_FBBitmapFont numberOfDotsWideForSymbol:[[self.symbols objectAtIndex:i] intValue]];
    ...
    [LAIGU_FBBitmapFont drawBackgroundWithDotType:self.dotType 
                                      color:self.offColor 
                                 edgeLength:self.edgeLength 
                                     margin:self.margin 
