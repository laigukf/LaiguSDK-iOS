...
#import "LAIGU_CafRecordWriter.h"

@interface LAIGU_CafRecordWriter()
{
...
@implementation LAIGU_CafRecordWriter


- (BOOL)createFileWithRecorder:(LAIGU_MLAudioRecorder*)recoder
{
    //建立文件
    recordPacketCount = 0;
...
- (BOOL)writeIntoFileWithData:(NSData*)data withRecorder:(LAIGU_MLAudioRecorder*)recoder inAQ:(AudioQueueRef)						inAQ inStartTime:(const AudioTimeStamp *)inStartTime inNumPackets:(UInt32)inNumPackets inPacketDesc:(const AudioStreamPacketDescription*)inPacketDesc
{
...
- (BOOL)completeWriteWithRecorder:(LAIGU_MLAudioRecorder*)recoder withIsError:(BOOL)isError
{
    if (mRecordFile) {
