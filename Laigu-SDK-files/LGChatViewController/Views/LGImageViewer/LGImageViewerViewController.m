...
@interface LGImageViewerViewController ()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@implementation LGImageViewerViewController
...
- (void)setImages:(NSArray *)images {
    ...
    for (UIImage * image in images) {
        [tempImageArray addObject:[LGImageUtil resizeImageToMaxScreenSize:image]];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LGImageCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:KCellReuseId forIndexPath:indexPath];
...
- (void)loadImageForIndexPath:(NSIndexPath *)indexPath forCell:(LGImageCollectionCell *)cell {
    NSString *imagePath = self.imagePaths[indexPath.row];
    if (self.images.count == 0) {
        [LGServiceToViewInterface downloadMediaWithUrlString:imagePath progress:nil completion:^(NSData *mediaData, NSError *error) {
            cell.imageView.image = [LGImageUtil resizeImageToMaxScreenSize:[UIImage imageWithData:mediaData]];
        }];
    } else {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[LGImageCollectionCell class] forCellWithReuseIdentifier:KCellReuseId];
    }
...
- (UIButton *)saveButton {
    if (!_saveButton) {
        ...
        [_saveButton setTitle:[LGBundleUtil localizedStringForKey:@"save_photo"] forState:(UIControlStateNormal)];
...
- (void)saveImage {
    ...
    UIImage *image = [(LGImageCollectionCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]] imageView].image;
    if (image) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *) error contextInfo:(void *)contextInfo {
    if (error) {
        [LGToast showToast:[LGBundleUtil localizedStringForKey:@"save_photo_error"] duration:0.25 window:self.view.window];
    } else {
        [LGToast showToast:[LGBundleUtil localizedStringForKey:@"save_photo_success"] duration:0.25 window:self.view.window];
    }
}
...
#pragma mark - ***
#pragma mark - LGImageCollectionCell

@interface LGImageCollectionCell() <UIScrollViewDelegate>
