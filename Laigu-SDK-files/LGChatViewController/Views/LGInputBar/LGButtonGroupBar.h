...
@class LGButtonGroupBar;

/**
 *  「LGButtonGroupBarDelegate」协议定义的方法是与「LGButtonGroupBar」对象进行交互的
 */
@protocol LGButtonGroupBarDelegate <NSObject>
 ...
 */
- (void)buttonGroup:(nonnull LGButtonGroupBar *)buttonGroupBar
     didClickButton:(nonnull UIButton *)sender
     formGroupIndex:(NSUInteger)index;

@end

@interface LGButtonGroupBar : UIView
...
/** 这个对象充当按钮组栏的委托 */
@property (weak, nonatomic, nullable  ) id<LGButtonGroupBarDelegate> delegate;

/**
 *  创建一个会显示一排按钮的矩形的视图
 *
 *  @return 如果初始化成功，将是一个「LGButtonGroupBar」对象，否则为 nil
 */
 ...
 *  @return 如果初始化成功，将是一个「LGButtonGroupBar」对象，否则为 nil
 */
- (_Nonnull instancetype)initWithFrame:(CGRect)frame buttons:(NSArray * _Nonnull)buttons delegate:(id<LGButtonGroupBarDelegate> _Nullable)delegate;
