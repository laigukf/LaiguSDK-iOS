//
//  LGKeyboardController.m
//  Laigu
//
//  Created by zhangshunxing on 16/4/15.
//  Copyright © 2016年 zhangshunxing. All rights reserved.
//

#import "LGKeyboardController.h"

static void * kLGKeyboardControllerKeyValueObservingContext = &kLGKeyboardControllerKeyValueObservingContext;

@interface LGKeyboardController () <UIGestureRecognizerDelegate>
                          ...
                          delegate:(id<LGKeyboardControllerDelegate>)delegate
{
    ...
    if (self = [self init]) {
        ...
        _delegate = delegate;
        _keyboardStatus = LGKeyboardStatusHide;
    }
    return self;
}
    ...
    if (keyboardView && !_isObserving) {
        [_keyboardView addObserver:self
                        forKeyPath:NSStringFromSelector(@selector(frame))
                           ...
                           context:kLGKeyboardControllerKeyValueObservingContext];
...
//                                self.keyboardView = subsubView;
    ...
    self.keyboardStatus = LGKeyboardStatusWillShow;
    [self setKeyboardViewHidden:NO];
...
{
    self.keyboardStatus = LGKeyboardStatusShowing;
...
{
    if (self.keyboardStatus == LGKeyboardStatusHide) {
        self.keyboardStatus = LGKeyboardStatusWillShow;
    }
    [self handleKeyboardNotification:notification completion:nil];
}
...
{
    self.keyboardStatus = LGKeyboardStatusWillHide;
}

- (void)didReceiveKeyboardDidHideNotification:(NSNotification *)notification
{
    self.keyboardStatus = LGKeyboardStatusHide;
    self.keyboardView = nil;
...
{
    if (context == kLGKeyboardControllerKeyValueObservingContext) {
