...
#import "LGMessage.h"
#import "LGDefinition.h"
#import "LGAgent.h"
#import "LGCardInfo.h"
#import "LGEnterprise.h"
 ...
 *  @param message 消息
 */
- (void)didReceiveLGMessages:(NSArray<LGMessage *> *)message;
 ...
 * @brief 来鼓SDK的配置管理类
 *
 * 开发者可以通过LGManager中提供的接口，对SDK进行配置；
 */
 ...
 *  开启来鼓服务
 *
 *  @warning App进入前台时，需要开启来鼓服务。开发者需要在AppDelegate.m中的applicationWillEnterForeground方法中，调用此接口，用于开启来鼓服务
 */
+ (void)openLaiguService;

/**
 *  关闭来鼓服务
 *
 *  @warning App退到后台时，需要关闭来鼓服务。开发者需要在AppDelegate.m中的applicationDidEnterBackground方法中，调用此接口，用于关闭来鼓服务
 */
+ (void)closeLaiguService;

/**
 * 设置用户的设备唯一标识，在AppDelegate.m的didRegisterForRemoteNotificationsWithDeviceToken系统回调中注册deviceToken。
 * App进入后台后，来鼓推送给开发者服务端的消息数据格式中，会有deviceToken的字段。
 *
...
/**
 @param deviceToken 去掉特殊符号和空格之后的字符串，若使用 swift 集成 LaiGuSDK 的时候，NSData 会被自动转为 Data，这个时候无法用上面的方法正确获取 deviceToken，需要使用这个方法。
 */
...
/**
 * 初始化SDK。来鼓建议开发者在AppDelegate.m中的系统回调didFinishLaunchingWithOptions中进行SDK初始化。
 * 如果成功返回一个顾客的信息，开发者可保存该clientId，绑定开发者自己的用户系统，下次使用setClientOnlineWithClientId进行上线
 *
 * @param appKey 来鼓提供的AppKey
...
/**
 获取消息所对应的企业 appkey
 */
+ (NSString *)appKeyForMessage:(LGMessage *)message;
...
+ (void)setScheduledAgentWithAgentId:(NSString *)agentId
                        ...
                        scheduleRule:(LGScheduleRules)scheduleRule;
 ...
 * @warning 需要顾客先上线，再上传顾客信息。如果开发者使用来鼓的开源界面，不需要调用此接口，使用 LGChatViewManager 中的 setClientInfo 配置用户自定义信息即可。
 * @warning 如果开发者使用「开源聊天界面」的接口来上线，则需要监听 LG_CLIENT_ONLINE_SUCCESS_NOTIFICATION「顾客成功上线」的广播（见 LGDefinition.h），再调用此接口
 */
 ...
 *  @warning 需要顾客上线之后，再调用此接口，具体请监听 LG_CLIENT_ONLINE_SUCCESS_NOTIFICATION「顾客成功上线」的广播，具体见 LGDefinition.h
 */
 ...
 */
+ (void)setCurrentClientOnlineWithSuccess:(void (^)(LGClientOnlineResult result, LGAgent *agent, NSArray<LGMessage *> *messages))success
                                  failure:(void (^)(NSError *error))failure
                   receiveMessageDelegate:(id<LGManagerDelegate>)receiveMessageDelegate;

/**
 * 根据来鼓的顾客id，登陆来鼓客服系统，并上线该顾客。请求成功后，该顾客将会出现在客服的对话列表中。
 *
 * @param clientId 来鼓的顾客id
 * @param result 上线结果，可以用作判断是否上线成功。
 * @param agent 上线成功后，被分配的客服实体
 * @param messages 当前对话的消息
 * @param receiveMessageDelegate 接收消息的委托代理
...
+ (void)setClientOnlineWithClientId:(NSString *)clientId
                            success:(void (^)(LGClientOnlineResult result, LGAgent *agent, NSArray<LGMessage *> *messages))success
                            failure:(void (^)(NSError *error))failure
             receiveMessageDelegate:(id<LGManagerDelegate>)receiveMessageDelegate;

/**
 * 根据开发者自定义的id，登陆来鼓客服系统，并上线该顾客。请求成功后，该顾客将会出现在客服的对话列表中。
 *
...
/**
 *  获取当前顾客的顾客id，开发者可保存该顾客id，下次使用setClientOnlineWithClientId接口来让该顾客登陆来鼓客服系统
 *
 *  @return 当前的顾客id
 *  
 */
...
/**
 * 来鼓将重新初始化一个新的顾客，该顾客没有任何历史记录及用户信息。开发者可选择将该id保存并与app的用户绑定。
 *
 ...
 * @warning 来鼓建议：退出聊天界面时，不要调用此接口；因为：如果设置了顾客离线，则客服发送的消息将会发送给开发者的推送服务器；如果没有设置顾客离线，开发者接受即时消息的代理收到消息，并收到新消息产生的notification；开发者可以监听此notification，用于显示小红点未读标记；
 */
...
+ (void)getDatabaseHistoryMessagesWithMsgDate:(NSDate *)msgDate
                               messagesNumber:(NSInteger)messagesNumber
                                       result:(void (^)(NSArray<LGMessage *> *messagesArray))result;
 ...
 *  清除所有来鼓的多媒体缓存
 *
 *  @param mediaSize 来鼓缓存多媒体的大小，以 M 为单位
 */
 ...
 */
+ (LGMessage *)sendTextMessageWithContent:(NSString *)content
                               completion:(void (^)(LGMessage *sendedMessage, NSError *error))completion;
 ...
 * @warning 需要在初始化成功后，且顾客是在线状态时调用才有效
 */
+ (LGMessage *)sendImageMessageWithImage:(UIImage *)image
                              completion:(void (^)(LGMessage *sendedMessage, NSError *error))completion;
 ...
 * @warning 需要在初始化成功后，且顾客是在线状态时调用才有效
 */
+ (LGMessage *)sendAudioMessage:(NSData *)audio
                     completion:(void (^)(LGMessage *sendedMessage, NSError *error))completion;
 ...
 *  @warning 来鼓建议：退出聊天界面时，不要调用此接口，顾客产生的对话如果长时间没有响应，来鼓后端将会结束这些超时的对话。
 ...
 */
+ (void)evaluateCurrentConversationWithEvaluation:(LGConversationEvaluation)evaluation
...
/**
 强制转人工
 */
+ (void)forceRedirectHumanAgentWithSuccess:(void (^)(LGClientOnlineResult result, LGAgent *agent, NSArray<LGMessage *> *messages))success
                                   failure:(void (^)(NSError *error))failure
                    receiveMessageDelegate:(id<LGManagerDelegate>)receiveMessageDelegate;
...
+ (void)getEnterpriseConfigDataWithCache:(BOOL)isLoadCache complete:(void(^)(LGEnterprise *, NSError *))action;
 ...
 */
+ (void)requestPreChatServeyDataIfNeedWithClientId:(NSString *)clientIdIn customizedId:(NSString *)customizedIdIn completion:(void(^)(LGPreChatData *data, NSError *error))block;
...
/**
 提交用户填写的留言工单
 */
+ (void)submitTicketForm:(NSString *)content userInfo:(NSDictionary *)userInfo completion:(void(^)(LGTicket *ticket, NSError *))block;
